...
class WriteUML25XMIFile extends Object {
  ...
  public WriteUML25XMIFile() {
    uuidNum = 1000000;
    // System.out.println("debug XMI2LabelSchemaWNamesDOM UMLModelClassifications.activeClasses:" +
    // UMLModelClassifications.activeClasses);
    classesToWrite = new ArrayList<>();
    classesToWrite = new ArrayList<>();
  ...
  public void writeXMIFile(String todaysDate) throws java.io.IOException {
    ...
    prXMLXMI = new PrintWriter(
        new OutputStreamWriter(new FileOutputStream(new File(lFileNameXMI)), "UTF-8"));

    writeXMIHdrXMI(todaysDate);
    // write classes
    for (Iterator<String> i = classesToWrite.iterator(); i.hasNext();) {
      String lClassName = i.next();
    ...
    // write associations
    ArrayList<Assoc> assocArr = new ArrayList<>(assocMap.values());
    for (Iterator<Assoc> i = assocArr.iterator(); i.hasNext();) {
      Assoc lAssoc = i.next();
      // prXMLXMI.println(" <ownedMember xmi:type=\"uml:Association\"" + " xmi:id=\"" + lAssoc.id +
      // " xmi:name=\"" + lAssoc.title + "\" visibility=\"public\">");
      ...
      prXMLXMI.println("          <packagedElement xmi:type='uml:Association' xmi:id='"
          + lAssoc.assocId + "' name='" + lAssoc.title + "'>");
      prXMLXMI.println("            <memberEnd xmi:idref='" + lAssoc.prop1Id + "'/>");
      prXMLXMI.println("            <memberEnd xmi:idref='" + lAssoc.prop2Id + "'/>");
      prXMLXMI.println("            <ownedEnd xmi:type='uml:Property' xmi:id='" + lAssoc.prop2Id
          + "' visibility='private' type='" + lAssoc.fromClassTitle + "' association='"
          + lAssoc.assocId + "'/>");
  ...
  public void writeXMIHdrXMI(String todaysDate) throws java.io.IOException {
    ...
    prXMLXMI
        .println("  <!-- Version: " + DMDocument.masterPDSSchemaFileDefn.ont_version_id + " -->");
    prXMLXMI.println("  <!-- Generated: " + todaysDate + " -->");
    prXMLXMI.println(
        "  <xmi:XMI xmi:version=\"2.1\" xmlns:uml=\"http://schema.omg.org/spec/UML/2.0\" xmlns:xmi=\"http://schema.omg.org/spec/XMI/2.1\">");
    prXMLXMI.println(
        "    <xmi:Documentation xmi:Exporter=\"InfomationModelExporter\" xmi:ExporterVersion=\"0.0.9\"/>");
    prXMLXMI.println(
        "    <uml:Model xmi:type='uml:Model' xmi:id='Information_Model' name='Information_Model'>");
    prXMLXMI.println(
        "      <packagedElement xmi:type='uml:Package' xmi:id='Information_Model_Package' name='Information_Model_Package'>");
    prXMLXMI.println(" ");
  }
  ...
  public void writeXMIClassXMI(DOMClass lClass) throws java.io.IOException {
    // System.out.println("\ndebug writeXMIClassXMI - Class - lClass.identifier:" +
    // lClass.identifier);
    // System.out.println("debug Class - lClass.identifier:" + lClass.identifier);
    prXMLXMI.println("        <packagedElement xmi:type='uml:Class' xmi:id='" + lClass.title
        + "' name='" + lClass.title + "'>");
    if (classesToWrite.contains(lClass.subClassOfTitle)) {
      // prXMLXMI.println(" <generalization xmi:type='uml:Generalization' xmi:idref='" +
      // lClass.subClassOfTitle + "' general='" + lClass.subClassOfTitle + "'/>");
      prXMLXMI.println("          <generalization xmi:type='uml:Generalization' xmi:id='"
          + lClass.subClassOfTitle + "_Generalization" + "' general='" + lClass.subClassOfTitle
          + "'/>");
    }
        ...
        if (lProp != null) {
          if (lProp.title.indexOf("__Method") > -1) {
            ...
            if (lProp.domObject != null && lProp.domObject instanceof DOMClass) {
              ...
              prXMLXMI.println("          <ownedAttribute xmi:type='uml:Property' xmi:id='"
                  + lAssoc.prop1Id + "' visibility='public'" + " type='" + lAssocClass.title
                  + "' association='" + lAssoc.assocId + "'" + "/>");
              // <ownedAttribute xmi:type='uml:Property'
              // xmi:id='_19_0_2_f470362_1577750889472_847048_5658' visibility='private'
              // type='Access_Service' association='_19_0_2_f470362_1577750889472_329572_5657'/>
            }
          ...
          if (lProp.title.indexOf("__Method") > -1) {
            if (lProp.domObject != null && lProp.domObject instanceof DOMClass) {
              ...
              prXMLXMI.println(
                  "          <ownedOperation xmi:type='uml:Operation' xmi:id='" + lMethodClass.title
                      + "' name='" + lMethodClass.title + "' visibility='public'>");
              // System.out.println("debug Method Class - lMethodClass.identifier:" +
              // lMethodClass.identifier);
              for (Iterator<DOMProp> k = lMethodClass.ownedAssocArr.iterator(); k.hasNext();) {
                DOMProp lParmProp = k.next();
                if (lParmProp != null) {
                  // System.out.println("debug CHECK ParmProp - lParmProp.identifier:" +
                  // lParmProp.identifier);
                  if ((lParmProp.title.indexOf("__Input") > -1)
                      || (lParmProp.title.indexOf("__Output") > -1)) {
                    String lDirection = "in";
                    if ((lParmProp.title.indexOf("__Output") > -1)) {
                      lDirection = "out";
                    }
                    ...
                    if (lParmProp.domObject != null
                      ...
                      // lParmClass.title + "\" name=\"" + lParmClass.title + "\"
                      // visibility=\"public\"/>");
                      prXMLXMI
                          .println("            <ownedParameter xmi:type='uml:Parameter' xmi:id='"
                              + lParmClass.title + "' name='" + lParmClass.title
                              + "' visibility='public' direction='" + lDirection + "'/>");
                    }
                  }
                }
              }
  ...
  public String getNextUUID() {
    uuidNum++;
    int ival = uuidNum;
    return "_" + Integer.toString(ival);
  }
}
